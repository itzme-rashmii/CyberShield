...
$(document).ready(function() {
    ...
    DEMO = !window.location.origin.startsWith('http') || (window.location.origin == "https://cybershielddemo.github.io");
    ...
    $.ajax({
        ...
        complete: function(response) {
            if(response.status === 404) {
                document.title = "CyberShield";
                document.body.hidden = true;
                throw new Error("CyberShield should be accessed ONLY at its server instance's address (e.g. http://127.0.0.1:8338)");
            }
                ...
                _USER = "";
                document.title = "CyberShield (unauthorized)";
                setTimeout(function() {
                    $("#login_link").click();
                }, 1000);
            }
...
function init(url, from, to) {
    ...
    document.title = "CyberShield (loading...)";
    $("body").loader("show");
    $("#main_container").toggleClass("hidden", true);
    $("#heatmap_container").hide();
    $("#calendar_container").hide();
        ...
        complete: function() {
            ...
            PAPAPARSE_COMPLETE_TIMER = setTimeout(function() {
                ...
                if (DEMO) {
                    ...
                    document.title = "CyberSheild (demo)";
                    ...
                    if (document.title.indexOf("unauthorized") === -1)
                        document.title = "CyberShield (" + period + ")";
...
// Initialize calendar elements when DOM is ready
$(document).ready(function() {
    // Create calendar modal if it doesn't exist
    if ($("#calendarModal").length === 0) {
        $('body').append(`
            <div id="calendarModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); justify-content:center; align-items:center; z-index:1000;">
    ...
    document.getElementById('closeCalendar').addEventListener('click', toggleCalendar);
...
function toggleCalendar() {
    if (calendarModal.style.display === 'none') {
        currentDate = new Date(); // Reset to current date when opening
        updateCalendar();
...
function updateCalendar() {
    ...
    document.querySelectorAll('#datepicker .day:not(.empty)').forEach(day => {
...
$(document).ready(function() {
    $('#showCalendar').click(toggleCalendar);
});
